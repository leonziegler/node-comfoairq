...
const debug = require('debug')('comfoq');
const debugVerbose = require('debug')('comfoq:details');
const debugError = require('debug')('comfoq:error');
...
class ComfoAirQ extends events {
  constructor(options) {
    ...
    before.debug.enabled = true;

    before.debugRaw = this._settings.debug;
    ...
    this._bridge.on('received', (data) => {
      ...
      if (data.kind == 40) {           // CnRpdoNotification
        ...
        data.result.data = after.analyze_CnNodeNotification(data);
